    ...
    for (k in 1:K) { # Loop for over time steps
      ...
      for (m in k:(length(L) - 1)) { # Loop over m periods at time k
        ...
        for (i in k:m) {
          ...
          sum.mu = sum.mu + dt * L.prev[i - k + 1] * sum.lam / (1 + dt * L.prev[i - k + 1])
        }

        ## Sum over q = 1 to p = M for the negative expression of mu
        for (q in 1:(length(L) - 1)) {
          neg.mu = Lambda[m - k + 1]^2 / 2
        }

        ## Sum over lambda * W for the final expression of Lm(Tk)
        for (q in 1:(length(L) - 1)) {
          Wkq = Wk * Lambda[m - k + 1]
        }
        ...
        L.new = L.prev[m - k + 1] * exp(mu * dt + sqrt(dt) * Wkq) # calculate new libor rate
        
        ## Save output in output dataframe
        output = rbind(output, data.frame(k = k, m = m, Lambda = Lambda[m - k + 1], `µm(Tk)` = mu, `W1(Tk)` = Wk[1], `W2(Tk)` = Wk[2], `Lm(Tk)` = L.new))
      }
    }
  }
